...
export default function ProyectosShow() {
    const { proyecto } = usePage().props as unknown as { proyecto: ProyectoShow };

    return (
        <AdminLayout title="Detalle del proyecto" description="Alcance, cliente asociado e historial de cambios.">
            <Head title={`Proyecto: ${proyecto.name}`} />
            <motion.div
            ...
            >
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{proyecto.name}</h1>
                        <span className={`mt-2 inline-flex px-2.5 py-1 rounded-full text-xs font-medium border ${statusBadge(proyecto.status)}`}>
                            {proyecto.status}
                        </span>
                    </div>
                </div>
                ...
                <div className="rounded-lg border border-slate-100 dark:border-white/[0.06] p-4">
                    <p className="text-xs uppercase text-slate-500 font-medium mb-1">Cliente</p>
                    <p className="text-slate-800 dark:text-slate-200">{proyecto.cliente?.name ?? 'Sin cliente'}</p>
                </div>

                {proyecto.description && (
                    <div className="rounded-lg border border-slate-100 dark:border-white/[0.06] p-4">
                        <p className="text-xs uppercase text-slate-500 font-medium mb-2">Descripción</p>
                        <p className="text-slate-700 dark:text-slate-300 text-sm whitespace-pre-wrap">{proyecto.description}</p>
                    </div>
                )}
